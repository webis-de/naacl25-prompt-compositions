...
from src.bias_detection.utils.experiments import (
    create_commonsense_prompts_from_base_template,
...
from src.prompt_templates import CommonSchema
...
def generate_output(
    ...
    model_output = model.generate(
        prompts,
        generation_params,
        custom_regex=generate_custom_json_regex(CommonSchema, COMMON_REGEX),
    ...
    extracted_output = [safe_get(obj, "isCorrect") for obj in fixed_output]
    predicted_labels = [1 if o == "yes" else 0 for o in extracted_output]
...
def main(args: argparse.Namespace) -> None:
    ...
    print(">>> Loading CommonSenseQA data ...")
    data_handler = DataHandler(datasets_to_load=["common_qa"])
    common_qa_data = data_handler.load_common_qa_data_as_dict()[split]
    script_logger.info(f"CommonSenseQA loaded: {common_qa_data.keys()}")
    ...
    for combination in prompt_component_combinations:
        ...
        for seed in args.seeds:
            ...
            prompts = create_commonsense_prompts_from_base_template(
                combination,
                common_qa_data,
                eos_token=llm.eos_token,
                description_type=COMMON_PROMPT_SETTINGS["description_type"],
                definition_type=COMMON_PROMPT_SETTINGS["definition_type"],
                stimulus_type=COMMON_PROMPT_SETTINGS["stimulus_type"],
                seed=seed,
                return_format_example=CommonSchema,
            )
            system_prompt = get_system_prompt(
                combination,
                COMMON_PROMPT_SETTINGS["system_prompt_type"],
                COMMON_PROMPT_SETTINGS["system_prompt_name"],
            )
            ...
            save_results(
                args, common_qa_data, output, label_dist, pred_label, seed, combination, prompts
            )
