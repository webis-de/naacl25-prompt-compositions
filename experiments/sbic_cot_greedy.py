...
def generate_prediction_output(
    ...
    sbic_bias_choices = [label.value for label in Binary_bias_labels]
    ...
    answer_token_distributions = model.get_output_distribution(model_output, sbic_bias_choices)
...
def main(args: argparse.Namespace) -> None:
    split_map = {"train": "train_sub_split_balanced", "test": "test", "dev": "dev", "val": "dev"}
    ...
    print(f">>> Loading SBIC {split} split...")
    data_handler = DataHandler(datasets_to_load=["sbic"])
    sbic_data = data_handler.load_sbic_data_as_dict()[split]
    script_logger.info(f"SBIC {split} split loaded: {sbic_data.keys()}")
    ...
    for combination in prompt_component_combinations:
        ...
        for seed in args.seeds:
            ...
            system_prompt = get_system_prompt(
                combination,
                SBIC_PROMPT_SETTINGS["system_prompt_type"],
                SBIC_PROMPT_SETTINGS["system_prompt_name"],
            )
            ...
            reasoning_prompts = create_bias_prompts_from_cot_template(
                ...
                data=sbic_data,
                description_type="sbic_cot",
                definition_type=SBIC_PROMPT_SETTINGS["definition_type"],
                stimulus_type=SBIC_PROMPT_SETTINGS["stimulus_type"],
                reasoning_step_type=SBIC_PROMPT_SETTINGS["reasoning_step_type"],
            ...
            prediction_prompts = create_bias_prompts_from_cot_template(
                prompt_components=combination,
                data=sbic_data,
                description_type=SBIC_PROMPT_SETTINGS["description_type"],
                definition_type=SBIC_PROMPT_SETTINGS["definition_type"],
                stimulus_type=SBIC_PROMPT_SETTINGS["stimulus_type"],
                reasoning_step_type=SBIC_PROMPT_SETTINGS["reasoning_step_type"],
                eos_token=llm.eos_token,
