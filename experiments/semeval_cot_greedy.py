...
from src.bias_detection.utils.experiments import (
    create_absa_prompts_from_cot_template,
...
from src.prompt_templates import ABSASchema
...
def generate_prediction_output(
    ...
    model_output = model.generate(
        prompts,
        generation_params,
        custom_regex=generate_custom_json_regex(ABSASchema, ABSA_RESPONSE_REGEX),
        system_prompt=system_prompt,
        use_tqdm=tqdm_flag,
    ...
    default_answer = {"sentiment": "positive"}
    fixed_output = [
    ...
    extracted_output = [safe_get(obj, "sentiment") for obj in fixed_output]
    predicted_labels = [1 if o == "positive" else 0 for o in extracted_output]

    answer_token_distributions = model.get_output_distribution(model_output, choices)
...
def main(args: argparse.Namespace) -> None:
    ...
    print(f">>> Loading Semeval {split} split...")
    data_handler = DataHandler(datasets_to_load=["semeval"])
    semeval_data = data_handler.load_semeval_data_as_dict()[split]
    script_logger.info(f"Semeval {split} split loaded: {semeval_data.keys()}")
    ...
    for combination in prompt_component_combinations:
        ...
        for seed in args.seeds:
            ...
            system_prompt = get_system_prompt(
                combination,
                SEMEVAL_PROMPT_SETTINGS["system_prompt_type"],
                SEMEVAL_PROMPT_SETTINGS["system_prompt_name"],
            )
            script_logger.info(f"System Prompt: {system_prompt!r}")

            reasoning_prompts = create_absa_prompts_from_cot_template(
                prompt_components=combination,
                data=semeval_data,
                description_type="absa_cot",
                definition_type=SEMEVAL_PROMPT_SETTINGS["definition_type"],
                stimulus_type=SEMEVAL_PROMPT_SETTINGS["stimulus_type"],
                reasoning_step_type=SEMEVAL_PROMPT_SETTINGS["reasoning_step_type"],
            ...
            prediction_prompts = create_absa_prompts_from_cot_template(
                prompt_components=combination,
                data=semeval_data,
                description_type=SEMEVAL_PROMPT_SETTINGS["description_type"],
                definition_type=SEMEVAL_PROMPT_SETTINGS["definition_type"],
                stimulus_type=SEMEVAL_PROMPT_SETTINGS["stimulus_type"],
                reasoning_step_type=SEMEVAL_PROMPT_SETTINGS["reasoning_step_type"],
                eos_token=llm.eos_token,
                seed=seed,
                return_format_example=ABSASchema,
