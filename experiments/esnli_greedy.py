...
from src.bias_detection.utils.experiments import (
    ...
    create_esnli_prompts_from_base_template,
    get_system_prompt,
    load_model,
    parse_bias_detection_arguments,
    should_skip_combination,
)
from src.prompt_templates import ESNLISchema
...
def generate_output(
    ...
    model_output = model.generate(
        prompts,
        generation_params,
        custom_regex=generate_custom_json_regex(ESNLISchema, ESNLI_REGEX),
    ...
    default_answer = {"answer": "contradiction"}
    fixed_output = [
    ...
    extracted_output = [safe_get(obj, "answer") for obj in fixed_output]
    predicted_labels = [1 if o == "contradiction" else 0 for o in extracted_output]
...
def save_results(
    ...
    pos_label, neg_label = "contradiction", "entailment"
...
def main(args: argparse.Namespace) -> None:
    ...
    print(">>> Loading ESNLI data ...")
    data_handler = DataHandler(datasets_to_load=["esnli"])
    esnli_data = data_handler.load_esnli_data_as_dict()[split]
    script_logger.info(f"ESNLI loaded: {esnli_data.keys()}")
    ...
    for combination in prompt_component_combinations:
        ...
        for seed in args.seeds:
            ...
            prompts = create_esnli_prompts_from_base_template(
                combination,
                esnli_data,
                eos_token=llm.eos_token,
                description_type=ESNLI_PROMPT_SETTINGS["description_type"],
                definition_type=ESNLI_PROMPT_SETTINGS["definition_type"],
                stimulus_type=ESNLI_PROMPT_SETTINGS["stimulus_type"],
                seed=seed,
                return_format_example=ESNLISchema,
            )
            system_prompt = get_system_prompt(
                combination,
                ESNLI_PROMPT_SETTINGS["system_prompt_type"],
                ESNLI_PROMPT_SETTINGS["system_prompt_name"],
            )
            ...
            save_results(
                args, esnli_data, output, label_dist, pred_label, seed, combination, prompts
            )
